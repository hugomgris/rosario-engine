...
namespace {
	// test helper function that creates a tmp json file, writes sent content to it, returns its path
...
TEST(ParticleConfigLoader, ParsesMultipleMenuTrailSets) {
	...
	EXPECT_EQ(config.menuTrails.size(), 2);
	...
	const std::string path = writeTempJson(R"JSON(
{
	...
	// Test should throw at the checkpoint x-y (a defined x REQUIRES a defined y for meuTrails)
	EXPECT_THROW(
